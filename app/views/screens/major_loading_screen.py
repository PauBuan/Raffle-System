...
Fewer winners → larger cards with dramatic entrance.
"""
...
class MajorLoadingScreen(QWidget):
    """Full-screen overlay for Major prize reveals."""
    ...
    def _reveal_next(self) -> None:
        if self._reveal_index >= len(self._cards):
            return
        self._cards[self._reveal_index].show()
        self._reveal_index += 1
        QTimer.singleShot(700, self._reveal_next)
